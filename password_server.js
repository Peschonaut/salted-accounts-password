...
const hashPassword = function (password, salt) {
    ...
    return bcryptHash(password + salt, Accounts._bcryptRounds);
};
...
var selectorForFastCaseInsensitiveLookup = function (fieldName, string) {
  // Performance seems to improve up to 4 prefix characters
    ...
    const orClause = _.map(generateCasePermutationsForString(prefix),
    (prefixPermutation) => {
        const selector = {};
        selector[fieldName] =
...
var generateCasePermutationsForString = function (string) {
    ...
    for (let i = 0; i < string.length; i++) {
        ...
        permutations = _.flatten(_.map(permutations, (prefix) => {
...
const checkForCaseInsensitiveDuplicates = function (fieldName, displayName, fieldValue, ownUserId) {
  // Some tests need the ability to add users with the same case insensitive
  // value, hence the _skipCaseInsensitiveChecksForTest check
    const skipCheck = _.has(Accounts._skipCaseInsensitiveChecksForTest, fieldValue);
...
Accounts.registerLoginHandler('password', (options) => {
  ...
  // Upgrade to bcrypt on successful login.
    const salted = hashPassword(options.password, user._id);
    Meteor.users.update(
    user._id,
        {
...
// method previously.
Meteor.methods({changePassword(oldPassword, newPassword) {
    ...
    const hashed = hashPassword(newPassword, user._id);
...
Accounts.setPassword = function (userId, newPlaintextPassword, options) {
    ...
    const update = {
        ...
        $set: {'services.password.bcrypt': hashPassword(newPlaintextPassword, user._id)}
    };
 ...
 * @locus Server
 * @param {String} userId The id of the user to update.
 * @param {String} oldSHA256Password The old SHA256 password for the user.
 * @param {Object} [options]
 * @param {Object} options.logout Logout all current connections with this userId (default: true)
 * @importFromPackage accounts-base
 */

var upgradePassword = function (userId, oldSHA256Password, options) {
    options = _.extend({logout: true}, options);
    ...
    const update = {
        ...
        $set: {'services.password.bcrypt': bcryptHash(oldSHA256Password + user._id, Accounts._bcryptRounds)}
    };
...
Meteor.methods({forgotPassword(options) {
    ...
    const caseSensitiveEmail = _.find(emails, email => email.toLowerCase() === options.email.toLowerCase());
...
Accounts.sendEnrollmentEmail = function (userId, email) {
  ...
  // before passing to template, update user object with new token
    ...
    const enrollAccountUrl = Accounts.urls.enrollAccount(token);
...
Meteor.methods({resetPassword(token, newPassword) {
    const self = this;
    ...
    () => {
      ...
      // of having a bad token set if things fail.
        ...
        const resetToOldToken = function () {
...
Accounts.sendVerificationEmail = function (userId, address) {
  ...
  // pick the first unverified address if we weren't passed an address.
    if (!address) {
        const email = _.find(user.emails || [],
                       e => !e.verified);
...
// and log them in.
Meteor.methods({verifyEmail(token) {
    const self = this;
    return Accounts._loginMethod(
    self,
    'verifyEmail',
    ...
    () => {
        ...
        const tokenRecord = _.find(user.services.email.verificationTokens,
                               t => t.token == token);
        if (!tokenRecord) {
            return {
                userId: user._id,
...
Accounts.addEmail = function (userId, newEmail, verified) {
  ...
  // that in this code anyway.
    ...
    const didUpdateOwnEmail = _.any(user.emails, (email, index) => {
...
const createUser = function (options) {
  ...
  // We update after insertion to make sure that we do not hurt other parts of the app
    if (options.password && options.password.digest && options.password.digest.length > 0) {
        const _options = options;
        _options.password = options.password.digest;
        userId = Accounts.insertUserDoc(_options, user);
        const hashed = bcryptHash(_options.password + userId, Accounts._bcryptRounds);
        Meteor.users.update({_id: userId}, {$set: {'services.password.bcrypt': hashed}});
    } else if (options.password) {
        userId = Accounts.insertUserDoc(options, user);
        const hashed = hashPassword(options.password, userId);
        Meteor.users.update({_id: userId}, {$set: {'services.password.bcrypt': hashed}});
    }
...
// Shared createUser function called from the createUser method, both
// if originates in client or server code. Calls user provided hooks,
// does the actual user insertion. Uses a the SHA256 hash instead of
// the plaintext password
//
// returns the user id
const createUserFromSHA256Hash = function (options) {
  // Unknown keys allowed, because a onCreateUserHook can take arbitrary
  // options.
    check(options, Match.ObjectIncluding({
        username: Match.Optional(String),
  ...
  // We update after insertion to make sure that we do not hurt other parts of the app
    if (options.password) {
        const hashed = bcryptHash(options.password + salt, Accounts._bcryptRounds);
        Meteor.users.update({_id: userId}, {$set: {'services.password.bcrypt': hashed}});
    }
  // Perform another check after insert, in case a matching user has been
  // inserted in the meantime
...
Meteor.methods({createUser(options) {
    const self = this;
    return Accounts._loginMethod(
    self,
    'createUser',
...
Accounts.createUserFromSHA256Hash = function (options, callback) {
    options = _.clone(options);

  // XXX allow an optional callback?
    if (callback) {
        throw new Error('Accounts.createUserFromSHA256Hash with callback not supported on the server yet.');
    }
