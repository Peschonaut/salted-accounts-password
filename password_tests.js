...
if (Meteor.isClient) {
  ...
  // now, that is this test.
        ...
        const addSkipCaseInsensitiveChecksForTest = function (value, test, expect) {
            Meteor.call('addSkipCaseInsensitiveChecksForTest', value);
        };

        const removeSkipCaseInsensitiveChecksForTest = function (value, test, expect) {
    ...
    // Hack because Tinytest does not clean the database between tests/runs
      ...
      loggedInAs(this.username, test, expect));
        ...
        const logoutStep = function (test, expect) {
        ...
        const loggedInUserHasEmail = function (someEmail, test, expect) {
            return expect((error) => {
                if (error) {
                    test.fail(error.message);
                }
                const user = Meteor.user();
                test.isTrue(user && _.some(user.emails, email => email.address === someEmail));
            });
        };
        const expectError = function (expectedError, test, expect) {
            return expect((actualError) => {
        ...
        const expectUserNotFound = function (test, expect) {
            return expectError(new Meteor.Error(403, 'User not found'), test, expect);
        };
        const waitForLoggedOutStep = function (test, expect) {
            pollUntil(expect, () => Meteor.userId() === null, 10 * 1000, 100);
        };
        const invalidateLoginsStep = function (test, expect) {
        ...
        const hideActualLoginErrorStep = function (test, expect) {
        ...
        const validateLoginsStep = function (test, expect) {
    ...
    // Log in a second connection as this user.
            ...
            function (test, expect) {
                const self = this;
                Accounts.changePassword(self.password, self.password2, expect((err) => {
                    test.isFalse(err);
                }));
            },
    ...
    // connection gets logged out.
            function (test, expect) {
                const self = this;
                pollUntil(expect, () => self.secondConn.userId() === null, 10 * 1000, 100);
            }
        ]);
    ...
    // resetPassword called on client with blank token
        ...
        this.token, this.newPassword, expect((error) => {
            test.isTrue(error);
        }));
            },
            function (test, expect) {
    ...
    // actually log in are.
      ...
      // we can't login with an invalid token
                const expectLoginError = expect((err) => {
                    test.isTrue(err);
                });
      ...
      // we can login with a valid token
                const expectLoginOK = expect((err) => {
                    test.isFalse(err);
                });
      ...
      // logged in.
                ...
                const expectSecondConnLoggedOut = expect((err, result) => {
                    test.isTrue(err);
                });

                const expectAccountsConnLoggedIn = expect((err, result) => {
                    test.isFalse(err);
                });
    ...
    // log in.
      ...
      // Test that deleting a user logs out that user's connections.
        ...
        testAsyncMulti('passwords - server onLogin hook', [
            function (test, expect) {
                Meteor.call('testCaptureLogins', expect((error) => {
                    test.isFalse(error);
                }));
            },
            function (test, expect) {
        ...
        testAsyncMulti('passwords - server onLogout hook', [
            function (test, expect) {
                Meteor.call('testCaptureLogouts', expect((error) => {
                    test.isFalse(error);
                }));
            },
            function (test, expect) {
        ...
        testAsyncMulti('passwords - client onLogout hook', [
            function (test, expect) {
                ...
                this.onLogout = Accounts.onLogout(() => {
        ...
        loggedInAs(this.username, test, expect));
            ...
            function (test, expect) {
                Meteor.loginWithPassword(this.username, 'incorrect', expect((error) => {
                    test.isTrue(error);
                }));
            },
            function (test, expect) {
            ...
            function (test, expect) {
                Meteor.loginWithPassword('no such user', 'incorrect', expect((error) => {
                    test.isTrue(error);
                }));
            },
            function (test, expect) {
        ...
        testAsyncMulti('passwords - client onLoginFailure hook', [
            function (test, expect) {
                ...
                this.onLoginFailure = Accounts.onLoginFailure(() => {
                    self.attempt = true;
                });
    ...
    // We are able to login with the old style credentials in the database.
            function (test, expect) {
                Meteor.loginWithPassword(this.username, 'abcdef', expect((error) => {
                    test.isFalse(error);
                }));
            },
            function (test, expect) {
                Meteor.call('testSRPUpgrade', this.username, expect((error) => {
                    test.isFalse(error);
                }));
            },
            logoutStep,
    // After the upgrade to bcrypt we're still able to login.
            ...
            logoutStep,
            function (test, expect) {
                Meteor.call('removeUser', this.username, expect((error) => {
                    test.isFalse(error);
                }));
            }
        ]);
    ...
    // Log in with the plaintext password handler, which should NOT upgrade us to bcrypt.
            function (test, expect) {
                Accounts.callLoginMethod({
                    ...
                    userCallback: expect((err) => {
                        test.isFalse(err);
                    })
                });
            },
            function (test, expect) {
                Meteor.call('testNoSRPUpgrade', this.username, expect((error) => {
                    test.isFalse(error);
                }));
            },
    // Changing our password should upgrade us to bcrypt.
            function (test, expect) {
                Accounts.changePassword('abcdef', 'abcdefg', expect((error) => {
                    test.isFalse(error);
                }));
            },
            function (test, expect) {
    ...
    // And after the upgrade we should be able to change our password again.
            function (test, expect) {
                Accounts.changePassword('abcdefg', 'abcdef', expect((error) => {
                    test.isFalse(error);
                }));
            },
            logoutStep
...
if (Meteor.isServer) {
    ...
    (test) => {
        ...
        test.throws(() => {
        // should fail the new user validators
            Accounts.createUser({username, profile: {invalid: true}});
        });
  ...
  // Accounts.config({forbidClientAccountCreation: true})
    ...
    (test) => {
        ...
        test.throws(() => {
            Meteor.call('login', {user: {username}, password: 'new-password'});
        }, /Incorrect password/);
    });
